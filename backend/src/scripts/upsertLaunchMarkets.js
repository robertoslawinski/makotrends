...
const upsertLaunchMarkets = async () => {
  if (!adminEmail) {
  ...
  for (const market of launchPredictions) {
  ...
  console.log("Launch markets are ready");
...
upsertLaunchMarkets()
  .catch((error) => {
    console.error("Launch market upsert failed:", error.message);
    process.exitCode = 1;
  })
  .finally(async () => {
